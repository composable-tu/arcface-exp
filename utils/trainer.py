...
class Trainer:
    ...
    def train_epoch(self, dataloader):
        ...
        progress_bar = tqdm(
            dataloader,
            desc=f'Training on {self.device}',
            leave=False,          # 每个 epoch 完成后清理这一行
            dynamic_ncols=True,   # 自动适配终端宽度，避免换行
            smoothing=0.1,
            mininterval=1.0,      # 限制刷新频率，避免终端异常刷屏/残留
        ...
        for batch_idx, (inputs, targets) in enumerate(progress_bar):
            ...
            # 打印进度 (每100个批次记录一次日志)
            if batch_idx % 100 == 0:
                self.logger.info(f'Batch [{batch_idx}/{len(dataloader)}], Loss: {loss.item():.4f}, Accuracy: {accuracy:.2f}%')
    ...
    def save_model(self, epoch, loss, accuracy, precision, recall, f1):
        ...
        if f1 > self.best_f1:
            ...
            self.logger.info(f'Best model updated to {best_save_path} (F1: {f1:.4f}, Accuracy: {accuracy:.2f}%, Loss: {loss:.4f})')
